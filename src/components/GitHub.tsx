...
export default function GitHub() {
  return (
    <section className="z-50 mx-auto my-16 max-w-4xl px-4 md:px-0">
      <h2 className="border-retro-purple dark:border-dark-purple mb-6 -ml-6 border-l-4 pl-4 text-xl font-bold uppercase">GitHub Showcase</h2>
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        {githubData.map((item, index) => (
          <div key={index}>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-xl font-bold">{item.name}</h3>
              <Link href={item.link} className="text-retro-purple dark:text-dark-purple text-xs hover:underline">
                View on GitHub
              </Link>
            </div>
